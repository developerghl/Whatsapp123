...
export default function DashboardLayout({
  ...
  const navigation = [
    ...
    { 
      name: 'Agency SMS',
      href: '/dashboard/sms',
      current: pathname === '/dashboard/sms' || pathname === '/dashboard/smsforhighlevel',
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="h-screen overflow-hidden bg-gray-50">
      ...
      <div className="flex h-[calc(100vh-64px)] overflow-hidden">
          {/* Sidebar - Modern Expanded Design */}
            ...
            <nav className="space-y-1">
                {navigation.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`${item.current ? 'text-white' : 'text-gray-500 group-hover:text-indigo-600'} mr-3 flex-shrink-0`}>
                        {item.icon}
                      </span>
                      <span className="flex-1">{item.name}</span>
                      {item.external && (
                        <svg className="w-4 h-4 ml-auto text-gray-400 group-hover:text-gray-600 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
